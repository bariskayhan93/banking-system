...
describe('GremlinService', () => {
  ...
  describe('executeQuery', () => {
    ...
    it('should throw error when client is not available', async () => {
      ...
      // Act & Assert
      await expect(service.executeQuery('any-query')).rejects.toThrow(
        'Gremlin client not available',
      );
    });
    ...
    it('should propagate errors from the query', async () => {
      ...
      // Act & Assert
      await expect(service.executeQuery('invalid-query')).rejects.toThrow();
    });
  });
